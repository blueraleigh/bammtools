...
##'
##' ed <- dtRates(ed, 0.01)
##' colors <- assignColorBreaks(ed$dtrates$rates[[1]]) # speciation rates
##' plot(ed, colorbreaks = colors, spex="s")
##' 
##' colors <- assignColorBreaks(ed$dtrates$rates[[1]] - ed$dtrates$rates[[2]]) # diversification rates
##' plot(ed, colorbreaks = colors, spex="netdiv")
##' @keywords graphics
##' @export
assignColorBreaks <- function(rates, NCOLORS = 64, logcolor = FALSE, method = c("linear","quantile","jenks"), JenksSubset = NULL) {
	method = match.arg(method, c("linear", "quantile", "jenks"))
	stopifnot(mode(rates) == "numeric")
	if (logcolor == FALSE) {
		if (method == "quantile") {
